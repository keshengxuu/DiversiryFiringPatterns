...
# values of pairwise (JEE,JEI)　
Jee = [0.8,0.5,0.50,1.1,2.4]
Jei = [0.82,0.35,0.15,0.045,0.275]
...
#        Time2
        ...
        with open("Gap00EISW_JeeVsJei_PSW001_Spikes/Spikes_%04d_Jee_%04d_Jei_%04d.txt"%(isim,jee,jei),'w') as f:
            writer=csv.writer(f,'excel')
            for sp in spikes:
                writer.writerow(['%.3f'%x for x in sp])

        np.savetxt("Gap00EISW_JeeVsJei_PSW001_AMP/AMP_%04d_Jee_%04d_Jei_%04d.txt"%(isim,jee,jei), V_t[::2,::5], fmt='%.18e',delimiter=' ',)

...
#        Time2
        ...
        with open("Gap005EISW_JeeVsJei_PSW001_Spikes/Spikes_%04d_Jee_%04d_Jei_%04d.txt"%(isim,jee,jei),'w') as f:
            writer=csv.writer(f,'excel')
            for sp in spikes:
                writer.writerow(['%.3f'%x for x in sp])

        np.savetxt("Gap005EISW_JeeVsJei_PSW001_AMP/AMP_%04d_Jee_%04d_Jei_%04d.txt"%(isim,jee,jei), V_t[::2,::5], fmt='%.18e',delimiter=' ',)
